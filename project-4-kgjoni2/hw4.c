...
void* next_chunk(void* c) { 
  ...
  if((c+chunk_size(c)) < sbrk(0))
    return ((void*)c+chunk_size(c)); // returns next chunk
  else 
    return 0;
}
int in_use(void *c) { 
  return (next_chunk(c) && ((*(size_t*)next_chunk(c)) & 1)); //returns 1 if chunk is in use
}
...
void sweep() {
  // TODO
  // COMMENTED OUT BECAUSE IMPLEMENTATION CAUSES SEG FAULT
  // size_t p = start;
  // size_t b = end;
  // while(p < b)
  //    if(is_marked(p))
